...
class SavedCardState extends State<SavedCard> {
  ...
  Widget build(BuildContext context) {
    _onValidate(CardDocument);
    return MaterialApp(
      title: 'Using Saved Credit Card',
...
void onValidate(document, BuildContext context) {
  charge = getRate();
  // Bill Customer
...
num getRate() {
  ...
  double Scharge =
      hours * double.parse('${brbuser.rate}') + 3; // + service charge
  double charge = double.parse(Scharge.toStringAsFixed(2));
  double cut = charge * .10; // 10% cut is ours
...
Future<void> takeMoney(String title, BuildContext context) async {
  // Creates a keyed charge
  var month = expiryDate.split('/');
  var amount = (double.parse('$charge') * 100).round();
  var card = cardNumber.split(' ');
  var newcardNumber = card.join('');
  print('This is card $newcardNumber');
  ...
  final response = await http.post(
    ...
    body: jsonEncode(<String, String>{
      'title': 'Keyed Charging',
      'exp_year': month.last,
      'exp_month': month.first,
      'amount': '$amount',
      'currency': 'usd',
      'statement_description': 'BrB rental payment',
      'email': '${brbuser.email}', // This is the user's information
      'phone_number': '${brbuser.phone}',
      'card_number': newcardNumber,
      'cvv': cvvCode,
      'metadata':
          '{"FullCustomerName" : " ${brbuser.userFirstName} ${brbuser.userLastName}"} ',
    }),
  );
  ...
  //if (response.statusCode == 403)
  {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Card Transfer failed'),
        actions: [
          TextButton(
            onPressed: () {
...
Future<int> refundCustomer(
  ...
  var cardData = data.map((d) => Cards.fromJson(d));
  var amount0 = (double.parse(amount) * 100).round();
  var month = cardData.first.expiryDate.split('/');
  var card = cardData.first.Number.split(' ');
  var cardNumber = card.join('');
  final response = await http.post(
    ...
    body: jsonEncode(<String, String>{
      ...
      'metadata':
          '{"FullCustomerName" : "${document['userFirstName']} ${document['userLastName']}"} ',
    }),
  );
  ...
  if (response.statusCode == 201) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: const Text('Card Data'),
        content: const Text('Invalid Card Data'),
        actions: [
          TextButton(
            onPressed: () {
              Navigator.pop(context);
