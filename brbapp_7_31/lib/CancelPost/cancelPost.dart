import 'dart:core';
import 'package:brbapp/assets/brbGlobals.dart';
import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:firebase_auth/firebase_auth.dart';
import 'package:flutter/material.dart';
import 'package:mailer/mailer.dart';
import 'package:mailer/smtp_server/gmail.dart';

import '../Card/card.dart';
//import 'package:syncfusion_flutter_datagrid/datagrid.dart';

/// Flutter code sample for [DataCancelPost].

class CancelPost extends StatefulWidget {
  const CancelPost({super.key});

  @override
  State<CancelPost> createState() => _CancelPostState();
}

class _CancelPostState extends State<CancelPost> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Property Listing'),
        backgroundColor: const Color(0xffff9900),
        leading: InkWell(
          onTap: () {
            Navigator.pushNamed(context, '/transaction');
          },
          child: const Icon(Icons.arrow_back_ios, color: Colors.black54),
        ),
      ),
      body: cancelPost(context),
    );
  }
}

Widget cancelPost(BuildContext context) {
  return StreamBuilder<QuerySnapshot>(
    stream: cancelStream(),
    builder: (context, AsyncSnapshot snapshot) {
      if (snapshot.connectionState == ConnectionState.waiting) {
        return const Center(child: CircularProgressIndicator());
      }
      if (!snapshot.hasData || snapshot.data.docs.isEmpty) {
        return const Center(child: Text('You have no Listing'));
      }

      return DataTable(
        columns: const [
          DataColumn(label: Text('Address')),
          DataColumn(label: Text('Cancel')),
          // DataColumn(label: Text('Delete')),
        ],
        rows: snapshot.data.docs.map<DataRow>((DocumentSnapshot document) {
          return DataRow(
            cells: [
              DataCell(Text(document['address'])),
              DataCell(
                // PUT BUTTON
                ElevatedButton(
                  style: ElevatedButton.styleFrom(
                    backgroundColor: Colors.orange,
                  ),
                  onPressed: () {
                    //  *******************************CONDITION

                    Timestamp start2 = Timestamp.fromDate(
                      DateTime.now(),
                    ); // should be greater than start
                    final DateTime start = DateTime.fromMicrosecondsSinceEpoch(
                      start2.microsecondsSinceEpoch,
                    );

                    //DateTime convertedDateTime = DateTime.parse(document['userStartDate'].toString());
                    // Timestamp convertedDateTimeStamp = Timestamp.fromDate(convertedDateTime);
                    Timestamp end2 = document['userStartDate'];
                    // Timestamp end2 = convertedDateTimeStamp;
                    final DateTime end = DateTime.fromMicrosecondsSinceEpoch(
                      end2.microsecondsSinceEpoch,
                    );
                    Duration total = end.difference(start); //
                    double Shours = double.parse(
                      (total.inMinutes / 60).toString(),
                    );

                    if (document['availability'] == "false" && Shours < 30) {
                      // cant canel within 30 minutes
                      showDialog(
                        context: context,
                        builder: (context) => AlertDialog(
                          title: const Text('Cancel Confirmation'),
                          content: const Text(
                            'You will be charged \$10 for cancelling a booked rental \n'
                            'half hour prior to appointment',
                          ),
                          actions: [
                            TextButton(
                              child: const Text('Cancel'),
                              onPressed: () => Navigator.pop(context),
                            ),
                            TextButton(
                              child: const Text(
                                'OK',
                                style: TextStyle(color: Colors.purple),
                              ),
                              onPressed: () async {
                                // send tis stream to the function

                                await charge(
                                  "10",
                                  context,
                                  document,
                                ); // then do reinbursment to customer
                                if (context.mounted) {
                                  refundCustomer(
                                    document['Due'].toString(),
                                    context,
                                    document,
                                  );
                                }
                                document.reference.update({
                                  'availability': 'canceled',
                                });
                                cancelEmail(document, context);
                              },
                            ),
                          ],
                        ),
                      );
                    } else {
                      document.reference.update({'availability': 'canceled'});
                      // In this case no need to refund anyone just delete the damn thing.
                      //refundCustomer(document['Due'].toString() , context, document);
                      //cancelEmail(document,context);// then do cash transaction for appointment that are !available
                      //document.reference.delete();
                      Navigator.pushNamed(context, '/confirmation');
                    }
                    // in either case send out the cancellation letter the charge is for

                    //1/3 time period before the appointment date.
                  },
                  child: const Text("Click"),
                ), // BUTTON
                onTap: () {
                  // We are not using the cell Gestures.
                },
              ),
              //DataCell(Text(document['firstName'])),
            ],
          );
        }).toList(), // dont call to list build it with mapping
      );
    },
  );
}

Stream<QuerySnapshot<Object?>> cancelStream() {
  var Fireuser2 =
      FirebaseAuth.instance.currentUser!.uid; // we have to call every time
  //var Fireuser2 = FirebaseAuth.instance.currentUser!.email;
  print('fireuser2 email ${Fireuser2.toString()}');
  String uid = Fireuser2.toString();
  var data = FirebaseFirestore.instance
      .collection('Users')
      .limit(10)
      .where('uid', isEqualTo: uid)
      .where('availability', isNotEqualTo: 'canceled')
      .get();
  return data.asStream();
}

void cancelEmail(DocumentSnapshot document, BuildContext context) async {
  // Note that using a username and password for gmail only works if
  // you have two-factor authentication enabled and created an App password.
  // Search for "gmail app password 2fa"
  // The alternative is to use oauth.
  String username = 'rohanfalcon111';
  String password = 'frnfdygqxnajseem';
  final smtpServer = gmail(username, password);
  // Use the SmtpServer class to configure an SMTP server:
  //final smtpServer = SmtpServer('smtp.gmail.com');
  // See the named arguments of SmtpServer for further configuration
  // options.

  Timestamp startDate0 =
      document['userStartDate']; //document['userStartDate'];
  Timestamp endDate0 = document['userEndDate']; // document['userEndDate'];
  DateTime startDate = startDate0.toDate();
  DateTime endDate = endDate0.toDate();

  //var end_date = document['endDate'];
  // Create our message.

  // final File file = await getImageFileFromAssets('brblogo.png');
  final message = Message()
    ..from = const Address('brb.parking@outlook.com', 'brb.parking@outlook.com')
    ..recipients.add(document['ownerEmail'])
    //..ccRecipients.addAll(['', ''])
    ..bccRecipients.add(document['ownerEmail']) // bcc not working
    ..subject = 'Rental Cancellation'
    ..html =
        '<p> Hi ${document['userFirstName']}<br> '
        'Your driveway rental for ${document['address']} starting ${fixDate(startDate)}'
        ' <br>and ending ${fixDate(endDate)} has been cancelled.<br> '
        'You Card wil be refunded \$ ${document['Due']}.'
        ' We apologise for any inconvenience.<br><br> </p>'
        '<h1>BrB Parking LLC</h1>'
        "<img src='data:image/png;base64,' width='50' height='50' alt='Company Logo'>";
  //..attachments.add(FileAttachment(File(file.path)))
  // ********************************************
  try {
    final sendReport = await send(
      message,
      smtpServer,
      timeout: const Duration(seconds: 15),
    );
    print('Message sent: ${sendReport.toString()}');
    if (context.mounted) {
      Navigator.pushNamed(context, '/confirmation');
    }
  } on MailerException catch (e) {
    print('Message not sent. Owner email ${brbuser.ownerEmail}');
    for (var p in e.problems) {
      print('Problem: ${p.code}: ${p.msg}');
    }
    // At this point we will have the card data if it is good
    // we can just send them back to the Vehicle screen for email
  }
  // DONE
}
